...
public:
    ...
    ~DRW_Spline() {
        for (auto p : controllist) delete p;
        controllist.clear();
        for (auto p : fitlist) delete p;
        fitlist.clear();
    }
    virtual void applyExtrusion(){}
...
public:
    ...
    ~DRW_HatchLoop() {
//        for (auto p : pollist) delete p;
//        pollist.clear();
        for (auto p : objlist) delete p;
        objlist.clear();
    }
...
protected:
    void parseCode(int code, dxfReader *reader);
    virtual bool parseDwg(DRW::Version version, dwgBuffer *buf, dwgBuffer *sBuf, duint32 bs = 0);
    virtual bool parseDwg(DRW::Version version, dwgBuffer* buf, duint32 bs=0);
...
public:
    ...
    ~DRW_Leader() {
        for (auto p : vertexlist) delete p;
        vertexlist.clear();
    }
