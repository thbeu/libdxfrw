...
bool dwgReader18::parseSysPage(duint8 *decompSec, duint32 decompSize){
    ...
    dwgCompressor comp;
    bool ret = comp.decompress18(tmpCompSec, decompSec, compSize, decompSize);
#ifdef DRW_DBG_DUMP
...
bool dwgReader18::parseDataPage(dwgSectionInfo si/*, duint8 *dData*/){
    ...
    for (auto it=si.pages.begin(); it!=si.pages.end(); ++it){
        ...
        dwgCompressor comp;
        if (!comp.decompress18(cData, oData, pi.cSize, pi.uSize)) {
            delete[]cData;
            return false;
        }
        delete[]cData;
...
bool dwgReader18::readDwgClasses(){
    ...
    if ((version > DRW::AC1021 && maintenanceVersion > 3) || version >= DRW::AC1032) { //2010+ or AC1032
        ...
        DRW_DBG("\n2010+ & MV> 3 or AC1032, height 32b: "); DRW_DBG(hSize);
    }
    duint32 bitSize = 0;
